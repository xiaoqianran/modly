...
test('Modal CLI tokens stay in a laptop session IPC, never settings.json or Generate', () => {
  ...
  assert.match(readRepo('electron/main/modal-session-ipc.ts'), /probeModalAsgi/)
  assert.equal(readRepo('electron/main/modal-session-ipc.ts').includes('ensureModalCpuAsgi'), false)
  assert.match(readRepo('scripts/deploy-modal.bat'), /uv venv \.venv-modal/)
  assert.match(readRepo('scripts/deploy-modal.bat'), /modal deploy/)
