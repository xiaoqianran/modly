...
test('Modal CLI tokens stay in a laptop session IPC, never settings.json or Generate', () => {
  ...
  assert.equal(readRepo('electron/main/modal-asgi-ensure.ts').includes('modal.cmd'), false)
  ...
  assert.match(readRepo('scripts/deploy-modal.bat'), /MODAL_TOKEN_ID/)
  ...
  assert.match(firstRun, /no browser/)
  assert.match(settings, /no browser/)
