...
class SettingsTests(unittest.TestCase):
    def test_defaults(self) -> None:
        ...
        self.assertEqual(s.gpu_scaledown_window, 60)
        ...
        self.assertEqual(gpu["scaledown_window"], 60)
        self.assertEqual(gpu["min_containers"], 0)
        self.assertEqual(gpu["buffer_containers"], 0)
        self.assertNotIn("single_use_containers", gpu)
        self.assertTrue(gpu["enable_memory_snapshot"])
        self.assertNotIn("experimental_options", gpu)
        fn = s.gpu_function_kwargs()
        self.assertTrue(fn["single_use_containers"])
        self.assertEqual(fn["min_containers"], 0)
